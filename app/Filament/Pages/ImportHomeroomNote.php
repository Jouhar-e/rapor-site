...
class ImportHomeroomNote extends Page implements HasTable
{
    ...
    protected static ?string $title = 'Import Catatan Wali Kelas';
    ...
    protected function finalizePreview(array $records, array $headers, array $state): void
    {
        ...
        session([
            'import_homeroom_note_data' => $records,
            'import_homeroom_note_headers' => $headers,
            'import_homeroom_note_academic_year_id' => $state['academic_year_id'],
            'import_homeroom_note_semester_id' => $state['semester_id'],
        ]);
    ...
    {
        $records = session('import_homeroom_note_data', []);

        if (empty($records)) {
            return;
        }

        $academicYearId = session('import_homeroom_note_academic_year_id');
        $semesterId = session('import_homeroom_note_semester_id');
        ...
        $result = $service->importHomeroomNotes($mapped);
        ...
        session()->forget([
            'import_homeroom_note_data', 'import_homeroom_note_headers',
            'import_homeroom_note_academic_year_id', 'import_homeroom_note_semester_id',
        ]);
        $this->step = 3;
    }
    ...
    {
        return $table
            ->query(ImportHistory::query()->where('type', 'homeroom-note'))
    ...
    protected function getHeaderActions(): array
    {
        ...
        if ($myClassIds->count() === 1) {
            $actions[] = Action::make('downloadTemplate')
                ...
                ->color('success')
                ->url(route('homeroom-note.template', ['class' => $myClassIds->first()]));
        } else {
            ...
            $actions[] = Action::make('downloadTemplate')
                ...
                ->action(function (array $data) {
                    $this->redirect(route('homeroom-note.template', ['class' => $data['class_id']]));
                });
        }

        $actions[] = Action::make('back')
            ->label('Kembali ke Input Catatan')
